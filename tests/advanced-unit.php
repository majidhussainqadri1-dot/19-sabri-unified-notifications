...
$att=file_get_contents(dirname(__DIR__).'/19-unified-notifications/includes/class-sun-attention-service.php');adv_check(false!==strpos($att,"'essential_only'")&&false!==strpos($att,'source_cap_reached'),'attention budgets and caps');adv_check(false!==strpos($att,'revoke_source')&&false!==strpos($att,'live_update'),'live/revocable projections');adv_check(false!==strpos($att,"'balanced'")&&false!==strpos($att,"'history_days' => 90"),'attention defaults present');
