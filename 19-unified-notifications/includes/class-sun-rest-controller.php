...
final class SUN_REST_Controller {
	...
	/** @param WP_REST_Request $r Request. @return WP_REST_Response|WP_Error */ public function delete_subscription($r){$result=$this->subscriptions->remove(get_current_user_id(),$r['id']);return is_wp_error($result)?$result:rest_ensure_response(array('success'=>true));}
	...
	/** @param WP_REST_Request $r Request. @return WP_REST_Response|WP_Error */ public function revoke_device($r){$result=$this->preferences->revoke_device(get_current_user_id(),$r['id']);return is_wp_error($result)?$result:rest_ensure_response(array('success'=>true));}

	/** @param WP_REST_Request $r Request. @return WP_REST_Response|WP_Error */
	public function ingest_event($r){$producer=sanitize_key((string)$r->get_header('x-sun-producer'));$timestamp=(string)$r->get_header('x-sun-timestamp');$signature=(string)$r->get_header('x-sun-signature');$raw=(string)$r->get_body();$limited=$this->rate_limit('producer:'.$producer,(int)apply_filters('sun_producer_rate_limit',120,$producer),MINUTE_IN_SECONDS);if(is_wp_error($limited)){return $limited;}$verified=$this->registry->verify_signature($producer,$timestamp,$signature,$raw);if(is_wp_error($verified)){return $verified;}$event=json_decode($raw,true);if(!is_array($event)){return new WP_Error('sun_json_invalid',__('Invalid JSON event.','sabri-unified-notifications'),array('status'=>400));}$event['producer']=$producer;$result=$this->notifications->ingest_event($event,'rest');return is_wp_error($result)?$result:rest_ensure_response($result);}
