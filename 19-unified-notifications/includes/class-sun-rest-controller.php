...
final class SUN_REST_Controller {
	...
	/** @param SUN_Notification_Service $notifications Notifications. @param SUN_Preferences $preferences Preferences. @param SUN_Subscriptions $subscriptions Scoped subscriptions. @param SUN_Delivery_Service $delivery Delivery. @param SUN_Reconciliation $reconciliation Reconciliation. @param SUN_Health $health Health. @param SUN_Auth $auth Auth. @param SUN_Producer_Registry $registry Registry. */
	public function __construct( SUN_Notification_Service $notifications, SUN_Preferences $preferences, SUN_Subscriptions $subscriptions, SUN_Delivery_Service $delivery, SUN_Reconciliation $reconciliation, SUN_Health $health, SUN_Auth $auth, SUN_Producer_Registry $registry ) {
		$this->notifications = $notifications; $this->preferences = $preferences; $this->subscriptions = $subscriptions; $this->delivery = $delivery; $this->reconciliation = $reconciliation; $this->health = $health; $this->auth = $auth; $this->registry = $registry;
	}
	...
	/** @return WP_REST_Response */ public function get_subscriptions(){return rest_ensure_response(array('scope_types'=>SUN_Subscriptions::scope_types(),'items'=>$this->subscriptions->list_for_user(get_current_user_id())));}
	/** @param WP_REST_Request $request Request. @return WP_REST_Response|WP_Error */ public function update_subscription($request){$limited=$this->rate_limit('subscriptions:'.get_current_user_id(),60,MINUTE_IN_SECONDS);if(is_wp_error($limited)){return $limited;}$result=$this->subscriptions->update(get_current_user_id(),$request->get_json_params()?:$request->get_params());return is_wp_error($result)?$result:rest_ensure_response($result);}
	...
	/** @return bool|WP_Error */ public function logged_in(){return is_user_logged_in()&&$this->auth->is_recipient_eligible(get_current_user_id())?true:new WP_Error('sun_auth_required',__('Authentication and current account eligibility are required.','sabri-unified-notifications'),array('status'=>401));}
