...
final class SUN_Renderer {
	...
	/** @param SUN_Notification_Service $notifications Notifications. @param SUN_Preferences $preferences Preferences. @param SUN_Subscriptions $subscriptions Scoped subscriptions. @param SUN_Auth $auth Current File 00 authorization. */
	public function __construct( SUN_Notification_Service $notifications, SUN_Preferences $preferences, SUN_Subscriptions $subscriptions, SUN_Auth $auth ) { $this->notifications=$notifications; $this->preferences=$preferences; $this->subscriptions=$subscriptions; $this->auth=$auth; }

	/** @return string */
	public function render_bell() {
		if ( ! is_user_logged_in() || ! $this->auth->is_recipient_eligible( get_current_user_id() ) ) { return ''; }
	...
	public function render_center( array $args = array() ) {
		...
		if ( ! $this->auth->is_recipient_eligible( get_current_user_id() ) ) { return '<div class="sun-notice">' . esc_html__( 'Notifications are not available for this account at this time.', 'sabri-unified-notifications' ) . '</div>'; }
	...
	public function render_settings() {
		...
		if ( ! $this->auth->is_recipient_eligible( get_current_user_id() ) ) { return '<div class="sun-notice">' . esc_html__( 'Notification settings are not available for this account at this time.', 'sabri-unified-notifications' ) . '</div>'; }
