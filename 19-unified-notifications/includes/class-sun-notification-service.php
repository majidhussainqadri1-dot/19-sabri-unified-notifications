...
final class SUN_Notification_Service {
	...
	/**
	 * Ingest an event atomically and fan out idempotent notifications.
	 *
	...
	public function ingest_event( array $event, $source = 'php' ) {
		...
		if ( $found ) {
			return array( 'event_public_id' => $found['public_id'], 'status' => 'duplicate', 'created' => 0, 'suppressed' => 0 );
		}
		...
		try {
			...
			foreach ( $event['recipients'] as $recipient ) {
				if ( ! $this->auth->is_recipient_eligible( (int) $recipient['user_id'] ) ) {
					++$suppressed;
					continue;
				}
	...
	private function create_notification( array $event, array $recipient, array $decision ) {
		...
		if ( $wpdb->get_var( $wpdb->prepare( "SELECT id FROM {$table} WHERE dedupe_key=%s LIMIT 1", $dedupe ) ) ) { // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared,WordPress.DB.DirectDatabaseQuery.DirectQuery
			return new WP_Error( 'sun_notification_duplicate', __( 'This notification already exists.', 'sabri-unified-notifications' ) );
		}
		...
		$ok = $wpdb->insert(
			...
			array(
				...
				'icon'            => $this->icon_for_category( $decision['category'] ),
				'title'           => $rendered['title'],
				'summary'         => $rendered['body'],
				'data_ciphertext' => $data_cipher,
				'deep_link'       => $deep_link ?: null,
				'deep_link_context'=> $event['deep_context'] ?: null,
				'status'          => 'unread',
		...
		if ( false === $ok ) {
			return new WP_Error( 'sun_notification_insert_failed', __( 'The notification could not be created.', 'sabri-unified-notifications' ) );
		}
	...
	/**
	 * List own notifications with cursor-like offset paging.
	 *
	...
	public function list_notifications( $user_id, array $args = array() ) {
		...
		$before = absint( $args['before_id'] ?? PHP_INT_MAX );
		$where  = array( 'recipient_id=%d', 'id<%d', "status<>'deleted'" );
		$params = array( absint( $user_id ), $before );
		if ( ! empty( $args['status'] ) && in_array( $args['status'], array( 'unread', 'read', 'archived' ), true ) ) {
		...
		$next_before_id = $rows ? (int) $rows[ count( $rows ) - 1 ]['id'] : 0;
		foreach ( $rows as &$row ) {
			$row['open_url'] = SUN_Deep_Link::wrapper_url( $row['public_id'] );
			unset( $row['id'] );
		}
		unset( $row );
		return array( 'items' => $rows, 'has_more' => $has_more, 'next_before_id' => $has_more ? $next_before_id : 0, 'unread_count' => $this->get_unread_count( $user_id ) );
	}
	...
	public function get_notification( $user_id, $public_id ) {
		...
		if ( ! $row || 'deleted' === $row['status'] ) {
			return new WP_Error( 'sun_notification_not_found', __( 'Notification not found.', 'sabri-unified-notifications' ), array( 'status' => 404 ) );
		}
		return $row;
	}
	...
	public function mutate( $user_id, $public_id, $action, $expected_version = null ) {
		...
		if ( null !== $expected_version && (int) $expected_version !== (int) $row['version'] ) {
	...
	public function bulk_mutate( $user_id, $action, array $filters = array() ) {
		global $wpdb;
		if ( ! in_array( $action, array( 'read', 'archive', 'unarchive' ), true ) ) {
			return new WP_Error( 'sun_bulk_action_invalid', __( 'Bulk action is invalid.', 'sabri-unified-notifications' ) );
		}
		$where = array( 'recipient_id=%d', "status<>'deleted'" ); $params = array( absint( $user_id ) );
		if ( ! empty( $filters['category'] ) ) { $where[]='category=%s'; $params[]=sanitize_key( $filters['category'] ); }
		$set = "status='read',read_at=%s"; $set_params = array( SUN_Database::now() );
		if ( 'archive' === $action ) { $set = "status='archived',archived_at=%s"; }
		if ( 'unarchive' === $action ) { $set = "status=IF(read_at IS NULL,'unread','read'),archived_at=NULL"; $set_params = array(); }
		$sql = 'UPDATE ' . SUN_Database::table( 'notifications' ) . " SET {$set},version=version+1,updated_at=%s WHERE " . implode( ' AND ', $where ) . ' LIMIT 1000';
		$query_params = array_merge( $set_params, array( SUN_Database::now() ), $params );
		$count = $wpdb->query( $wpdb->prepare( $sql, $query_params ) ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared,WordPress.DB.DirectDatabaseQuery.DirectQuery
		SUN_Audit::record( 'notifications_bulk_' . $action, 'notification_set', (string) $user_id, array( 'count' => (int) $count, 'purpose' => 'user_action' ), $user_id );
		return (int) $count;
	}
	...
	public function get_unread_count( $user_id ) {
		global $wpdb;
		if ( ! $this->auth->is_recipient_eligible( $user_id ) ) { return 0; }
	...
	public function expire_due() {
		global $wpdb;
		return (int) $wpdb->query( $wpdb->prepare( "UPDATE " . SUN_Database::table( 'notifications' ) . " SET status='expired',updated_at=%s,version=version+1 WHERE status NOT IN ('expired','deleted') AND expires_at IS NOT NULL AND expires_at<=%s", SUN_Database::now(), SUN_Database::now() ) ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery
	}
	...
	private function variables( array $event ) {
		...
		return array(
			...
			'action_name' => (string) ( $data['action_name'] ?? $event['event_type'] ),
			'summary'     => (string) ( $data['summary'] ?? __( 'A new update is available.', 'sabri-unified-notifications' ) ),
	...
	private function icon_for_category( $category ) {
		$map = array( 'security'=>'shield','safety'=>'warning','clinic'=>'heart','publishing'=>'edit','learning'=>'book','social'=>'users','marketplace'=>'store','messages'=>'message','media'=>'play','system'=>'settings' );
