...
final class SUN_Automation_Service {
    ...
    public function upsert_rule( $user_id, array $input ) {
        ...
        $table = SUN_Database::table( 'notification_rules' ); $now = SUN_Database::now(); $existing = $public_id ? $wpdb->get_row( $wpdb->prepare( "SELECT id,version FROM {$table} WHERE public_id=%s AND user_id=%d LIMIT 1", $public_id, $user_id ), ARRAY_A ) : null;
        ...
        $data = array( 'user_id' => $user_id, 'name' => $name, 'trigger_type' => $type, 'trigger_json' => wp_json_encode( $trigger ), 'action_json' => wp_json_encode( $action ), 'enabled' => ! empty( $input['enabled'] ) ? 1 : 0, 'version' => (int) ( $existing['version'] ?? 0 ) + 1, 'updated_at' => $now );
    ...
    public function on_event_processed( array $event, $created = 0, $suppressed = 0 ) {
        global $wpdb; foreach ( (array) ( $event['recipients'] ?? array() ) as $recipient ) { $user_id = absint( $recipient['user_id'] ?? 0 ); if ( $user_id < 1 ) { continue; }
            ...
            foreach ( (array) $rules as $rule ) {
                $trigger = json_decode( (string) $rule['trigger_json'], true ) ?: array(); if ( ! $this->matches( (string) $rule['trigger_type'], $trigger, $event ) ) { continue; }
                $action = json_decode( (string) $rule['action_json'], true ) ?: array(); $result = $this->execute_rule_action( $user_id, (string) $rule['public_id'], $action, $event );
                $wpdb->update( SUN_Database::table( 'notification_rules' ), array( 'last_matched_at' => SUN_Database::now(), 'updated_at' => SUN_Database::now() ), array( 'id' => (int) $rule['id'] ) );
                do_action( 'sun_notification_rule_matched', $user_id, $rule['public_id'], $action, $event, $result );
                SUN_Audit::record( is_wp_error($result)?'notification_rule_action_failed':'notification_rule_action_executed', 'notification_rule', $rule['public_id'], array( 'event_type' => $event['event_type'], 'action_type' => sanitize_key( (string) ( $action['type'] ?? '' ) ), 'result' => is_wp_error($result)?sanitize_key($result->get_error_code()):(string)($result['status']??'executed'), 'purpose' => 'user_automation' ), $user_id );
                SUN_Trace_Service::record( (string) ( $event['trace_id'] ?? $rule['public_id'] ), 'automation_action', is_wp_error($result)?'failed':'executed', array( 'rule_id'=>$rule['public_id'], 'action_type'=>sanitize_key((string)($action['type']??'')) ) );
            }
        }
    }

    /** @param int $user_id User ID. @param string $rule_id Rule ID. @param array<string,mixed> $action Action. @param array<string,mixed> $event Event. @return array<string,mixed>|WP_Error */
    private function execute_rule_action( $user_id, $rule_id, array $action, array $event ) {
        global $wpdb; $type=sanitize_key((string)($action['type']??'notify'));
        $note=$wpdb->get_row($wpdb->prepare('SELECT n.id,n.public_id FROM '.SUN_Database::table('notifications').' n WHERE n.recipient_id=%d AND n.producer=%s AND n.event_id=%s AND n.status NOT IN (\'deleted\',\'expired\') ORDER BY n.id DESC LIMIT 1',absint($user_id),sanitize_key((string)($event['producer']??'')),sanitize_text_field((string)($event['event_id']??''))),ARRAY_A);
        if('notify'===$type){return $note?array('status'=>'projection-retained','notification_id'=>$note['public_id']):new WP_Error('sun_rule_notification_missing',__('The matched event has no notification projection to retain.','sabri-unified-notifications'));}
        if('mark_needs_action'===$type){if(!$note){return new WP_Error('sun_rule_notification_missing',__('The matched event has no notification projection.','sabri-unified-notifications'));}$updated=$this->attention->mutate_state($user_id,(string)$note['public_id'],'needs_action');return is_wp_error($updated)?$updated:array('status'=>'marked-needs-action','notification_id'=>$note['public_id']);}
        if('digest'===$type){if(!$note){return new WP_Error('sun_rule_notification_missing',__('The matched event has no notification projection.','sabri-unified-notifications'));}$frequency=sanitize_key((string)($action['frequency']??'daily'));$when=new DateTimeImmutable('now',new DateTimeZone('UTC'));if('weekly'===$frequency){$when=$when->modify('next monday')->setTime(8,0);}elseif('immediate'===$frequency){$when=$when;}else{$when=$when->modify('+1 day')->setTime(8,0);}$when=apply_filters('sun_notification_rule_digest_time',$when,$frequency,$user_id,$event,$rule_id);if(!$when instanceof DateTimeImmutable){return new WP_Error('sun_rule_digest_time_invalid',__('The digest schedule is invalid.','sabri-unified-notifications'));}$count=$wpdb->query($wpdb->prepare("UPDATE ".SUN_Database::table('deliveries')." SET scheduled_at=%s,next_attempt_at=%s,digest_key=%s,updated_at=%s WHERE notification_id=%d AND status IN ('queued','failed')",$when->format('Y-m-d H:i:s'),$when->format('Y-m-d H:i:s'),'rule:'.$rule_id.':'.$frequency,$when->format('Y-m-d H:i:s'),(int)$note['id']));return array('status'=>'digest-scheduled','deliveries'=>(int)$count,'notification_id'=>$note['public_id']);}
        if(in_array($type,array('save_reference','calendar_handoff','owner_action'),true)){$owner_result=apply_filters('sun_notification_rule_owner_action',null,$type,$action,$event,$user_id,$rule_id);return null===$owner_result?new WP_Error('sun_rule_owner_action_unavailable',__('The native owner is not available to execute this automation action.','sabri-unified-notifications'),array('status'=>503)):$owner_result;}
    ...
    private function matches( $type, array $trigger, array $event ) { $event_type = (string) ( $event['event_type'] ?? '' ); $category = sanitize_key( (string) ( $event['category'] ?? '' ) ); $producer = sanitize_key( (string) ( $event['producer'] ?? '' ) ); switch ( $type ) { case 'event_type': $pattern = (string) ( $trigger['pattern'] ?? '' ); return $pattern && ( $event_type === $pattern || ( substr( $pattern, -2 ) === '.*' && 0 === strpos( $event_type, substr( $pattern, 0, -1 ) ) ) ); case 'category': return sanitize_key( (string) ( $trigger['category'] ?? '' ) ) === $category; case 'source': return sanitize_key( (string) ( $trigger['producer'] ?? '' ) ) === $producer; case 'saved_search': return 'Search.SavedSearchMatched' === $event_type && (string) ( $event['subject']['id'] ?? '' ) === (string) ( $trigger['search_id'] ?? '' ) && ( '' === (string)($trigger['owner']??'') || sanitize_key((string)($trigger['owner']??'')) === $producer || sanitize_key((string)($trigger['owner']??'')) === sanitize_key((string)($event['owner']??'')) ); case 'correction': return false !== stripos( $event_type, 'Correction' ) || false !== stripos( $event_type, 'Retraction' ); case 'learning': return 0 === strpos( $event_type, 'Learning.' ); case 'clinic': return 0 === strpos( $event_type, 'Clinic.' ); case 'research': return 0 === strpos( $event_type, 'Research.' ) || 0 === strpos( $event_type, 'Knowledge.' ); } return false; }

    /** @param string $type Type. @param array<string,mixed> $trigger Trigger. @return array<string,mixed>|WP_Error */
    private function sanitize_trigger( $type, array $trigger ) { switch ( $type ) { case 'event_type': $pattern = substr( sanitize_text_field( (string) ( $trigger['pattern'] ?? '' ) ), 0, 191 ); if ( ! preg_match( '/^[A-Z][A-Za-z0-9]*(?:\.[A-Z*][A-Za-z0-9*]*)+$/', $pattern ) ) { return new WP_Error( 'sun_rule_pattern_invalid', __( 'The event rule pattern is invalid.', 'sabri-unified-notifications' ), array( 'status' => 400 ) ); } return array( 'pattern' => $pattern ); case 'category': return array( 'category' => sanitize_key( (string) ( $trigger['category'] ?? '' ) ) ); case 'source': return array( 'producer' => sanitize_key( (string) ( $trigger['producer'] ?? '' ) ) ); case 'saved_search': $owner = substr( sanitize_key( (string) ( $trigger['owner'] ?? 'file26' ) ), 0, 100 ); $id = substr( sanitize_text_field( (string) ( $trigger['search_id'] ?? '' ) ), 0, 191 ); if ( '' === $id ) { return new WP_Error( 'sun_saved_search_invalid', __( 'A saved-search identifier is required.', 'sabri-unified-notifications' ), array( 'status' => 400 ) ); } return array( 'owner' => $owner, 'search_id' => $id ); default: return array(); } }
