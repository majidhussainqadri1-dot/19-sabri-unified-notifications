...
final class SUN_Preferences {
	...
	/**
	 * Return effective preference for one category/channel.
	 *
	...
	/**
	 * List effective preferences for all categories/channels.
	 *
	 * @param int $user_id User ID.
	...
	public function get_all( $user_id ) {
		...
		foreach ( $categories as $category ) {
			foreach ( $channels as $channel ) {
				$out[] = $this->get( $user_id, $category, $channel );
			}
		}
		return $out;
	}
	...
	/**
	 * Update one preference with optimistic concurrency.
	 *
	 * @param int                 $user_id User ID.
	...
	public function update( $user_id, array $input ) {
		...
		if ( $version !== (int) $current['version'] ) {
			return new WP_Error( 'sun_preference_conflict', __( 'Your notification settings changed in another session. Reload and try again.', 'sabri-unified-notifications' ), array( 'status' => 409 ) );
		}
		$essential = in_array( $category, array( 'security', 'safety', 'system' ), true );
		$enabled   = ! empty( $input['enabled'] );
		if ( $essential && 'in_app' === $channel ) {
			$enabled = true;
		}
		$digest = sanitize_key( (string) ( $input['digest_frequency'] ?? 'immediate' ) );
		if ( ! in_array( $digest, array( 'immediate', 'daily', 'weekly' ), true ) || $essential ) {
			$digest = 'immediate';
		}
		$timezone = $this->valid_timezone( (string) ( $input['timezone'] ?? $current['timezone'] ) );
		$start    = $this->valid_time( (string) ( $input['quiet_start'] ?? '22:00' ) );
		$end      = $this->valid_time( (string) ( $input['quiet_end'] ?? '07:00' ) );
		$now      = SUN_Database::now();
		$table    = SUN_Database::table( 'preferences' );
		$id       = (int) $wpdb->get_var(
			$wpdb->prepare( "SELECT id FROM {$table} WHERE user_id=%d AND category=%s AND channel=%s", $user_id, $category, $channel )
		...
		$data = array(
			...
			'quiet_enabled'    => ! empty( $input['quiet_enabled'] ) && ! $essential ? 1 : 0,
			'quiet_start'      => $start,
			'quiet_end'        => $end,
			'timezone'         => $timezone,
			'consent_source'   => sanitize_key( (string) ( $input['consent_source'] ?? 'settings' ) ),
		...
		SUN_Audit::record( 'preference_changed', 'preference', $user_id . ':' . $category . ':' . $channel, array( 'purpose' => 'user_choice' ), $user_id );
		do_action( 'sun_notification_preference_changed', $user_id, $category, $channel, $data );
		return $this->get( $user_id, $category, $channel );
	}

	/**
	 * Calculate the earliest lawful external delivery time, respecting DST.
	 *
	 * @param int                 $user_id User ID.
	 * @param string              $category Category.
	 * @param string              $channel Channel.
	 * @param bool                $mandatory Mandatory alert.
	 * @param DateTimeImmutable|null $now Current instant.
	 * @return DateTimeImmutable
	 */
	public function next_delivery_time( $user_id, $category, $channel, $mandatory = false, $now = null ) {
		$pref = $this->get( $user_id, $category, $channel );
		$tz   = new DateTimeZone( $this->valid_timezone( (string) $pref['timezone'] ) );
		$now  = $now instanceof DateTimeImmutable ? $now->setTimezone( $tz ) : new DateTimeImmutable( 'now', $tz );
		if ( $mandatory || empty( $pref['quiet_enabled'] ) ) {
			return $now->setTimezone( new DateTimeZone( 'UTC' ) );
		}
		$start = $this->time_on_date( $now, (string) $pref['quiet_start'] );
		$end   = $this->time_on_date( $now, (string) $pref['quiet_end'] );
		if ( $start == $end ) {
			return $now->setTimezone( new DateTimeZone( 'UTC' ) );
		}
		if ( $end < $start ) {
	...
	public function digest_schedule( $user_id, $category, $channel, DateTimeImmutable $base ) {
		$pref = $this->get( $user_id, $category, $channel );
		$freq = (string) $pref['digest_frequency'];
		$tz   = new DateTimeZone( $this->valid_timezone( (string) $pref['timezone'] ) );
	...
	public function register_device( $user_id, array $input ) {
		...
		if ( $user_id < 1 || strlen( $token ) < 20 || strlen( $token ) > 8192 ) {
			return new WP_Error( 'sun_device_token_invalid', __( 'The push device token is invalid.', 'sabri-unified-notifications' ), array( 'status' => 400 ) );
		}
		$cipher = SUN_Crypto::encrypt( $token );
		...
		if ( $existing && (int) $existing['user_id'] !== $user_id ) {
			return new WP_Error( 'sun_device_token_owned', __( 'This notification device is already bound to another account.', 'sabri-unified-notifications' ), array( 'status' => 409 ) );
		}
		...
		if ( $existing ) {
			...
			if ( false === $updated ) {
				return new WP_Error( 'sun_device_write_failed', __( 'The notification device could not be saved.', 'sabri-unified-notifications' ), array( 'status' => 500 ) );
			}
			...
			$inserted = $wpdb->insert(
				$table,
				array(
					'public_id'=>$public_id,'user_id'=>$user_id,'provider'=>$provider,'platform'=>$platform,'token_hash'=>$hash,'token_ciphertext'=>$cipher,
					'status'=>'active','last_seen_at'=>$now,'created_at'=>$now,'updated_at'=>$now,
				)
			); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery
			if ( false === $inserted ) {
				return new WP_Error( 'sun_device_write_failed', __( 'The notification device could not be saved.', 'sabri-unified-notifications' ), array( 'status' => 500 ) );
	...
	public function revoke_device( $user_id, $public_id ) {
		global $wpdb;
		$updated = $wpdb->update(
			SUN_Database::table( 'devices' ),
			array( 'status' => 'revoked', 'updated_at' => SUN_Database::now() ),
			array( 'user_id' => absint( $user_id ), 'public_id' => sanitize_text_field( $public_id ) )
		);
		if ( $updated ) {
			SUN_Audit::record( 'device_revoked', 'device', $public_id, array( 'purpose' => 'user_choice' ), $user_id );
			return true;
		}
	...
	public function categories() {
		return array( 'security', 'safety', 'clinic', 'publishing', 'learning', 'social', 'marketplace', 'messages', 'media', 'system', 'marketing' );
	}

	/** @return string[] */
	public function channels() {
		return array( 'in_app', 'email', 'push', 'sms' );
	}
	...
	private function defaults( $user_id, $category, $channel ) {
		$claims    = $this->auth->assertions( $user_id );
		$essential = in_array( $category, array( 'security', 'safety', 'system' ), true );
		$enabled   = 'in_app' === $channel || ( 'email' === $channel && $essential );
		if ( 'email' === $channel && empty( $claims['email_verified'] ) ) {
			$enabled = false;
		}
		if ( 'sms' === $channel && empty( $claims['phone_verified'] ) ) {
			$enabled = false;
		}
		$timezone = $claims['timezone'] ?: wp_timezone_string();
		return array(
			'user_id'          => absint( $user_id ),
			'category'         => $category,
			'channel'          => $channel,
			...
			'quiet_end'        => '07:00:00',
			'timezone'         => $this->valid_timezone( $timezone ),
			'version'          => 0,
		);
	}

	/** @param string $timezone Timezone. @return string */
	...
	/** @param string $time Time. @return string */
	private function valid_time( $time ) {
		return preg_match( '/^(?:[01]\d|2[0-3]):[0-5]\d(?::[0-5]\d)?$/', $time ) ? ( strlen( $time ) === 5 ? $time . ':00' : $time ) : '22:00:00';
	}

	/** @param DateTimeImmutable $date Date. @param string $time Time. @return DateTimeImmutable */
	private function time_on_date( DateTimeImmutable $date, $time ) {
		$parts = array_map( 'intval', explode( ':', $this->valid_time( $time ) ) );
		return $date->setTime( $parts[0], $parts[1], $parts[2] ?? 0 );
	}
}
