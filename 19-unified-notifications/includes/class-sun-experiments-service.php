<?php
/** Notification policy simulator, shadow evaluation and deterministic canary rollout. */
if ( ! defined( 'ABSPATH' ) ) { exit; }

final class SUN_Experiments_Service {
    /** @return string[] */ public function types() { return array( 'simulator', 'shadow', 'canary' ); }
    /** @param int $owner_id Owner. @param array<string,mixed> $input Input. @return array<string,mixed>|WP_Error */
    public function create( $owner_id, array $input ) { global $wpdb; $type = sanitize_key( (string) ( $input['experiment_type'] ?? 'simulator' ) ); if ( ! in_array( $type, $this->types(), true ) ) { return new WP_Error( 'sun_experiment_type_invalid', __( 'Notification experiment type is invalid.', 'sabri-unified-notifications' ), array( 'status' => 400 ) ); } $policy_key = substr( sanitize_key( (string) ( $input['policy_key'] ?? 'candidate' ) ), 0, 191 ); $rollout = max( 0, min( 100, (float) ( $input['rollout_percent'] ?? 0 ) ) ); if ( 'canary' !== $type ) { $rollout = 0; } $config = $this->sanitize_config( (array) ( $input['config'] ?? array() ) ); $public_id = SUN_Database::uuid(); $now = SUN_Database::now(); $ok = $wpdb->insert( SUN_Database::table( 'experiments' ), array( 'public_id' => $public_id, 'owner_id' => absint( $owner_id ), 'experiment_type' => $type, 'policy_key' => $policy_key, 'config_json' => wp_json_encode( $config ), 'status' => 'draft', 'rollout_percent' => $rollout, 'metrics_json' => wp_json_encode( array( 'evaluated' => 0, 'differences' => 0, 'critical_misses' => 0 ) ), 'created_at' => $now, 'updated_at' => $now ) ); return false === $ok ? new WP_Error( 'sun_experiment_write_failed', __( 'The notification experiment could not be created.', 'sabri-unified-notifications' ), array( 'status' => 500 ) ) : $this->get( $public_id ); }
    /** @param string $public_id ID. @return array<string,mixed>|WP_Error */
    public function get( $public_id ) { global $wpdb; $row = $wpdb->get_row( $wpdb->prepare( 'SELECT * FROM ' . SUN_Database::table( 'experiments' ) . ' WHERE public_id=%s LIMIT 1', sanitize_text_field( $public_id ) ), ARRAY_A ); if ( ! $row ) { return new WP_Error( 'sun_experiment_not_found', __( 'Notification experiment not found.', 'sabri-unified-notifications' ), array( 'status' => 404 ) ); } $row['config'] = json_decode( (string) $row['config_json'], true ) ?: array(); $row['metrics'] = json_decode( (string) $row['metrics_json'], true ) ?: array(); unset( $row['config_json'], $row['metrics_json'], $row['id'] ); return $row; }
    /** @param string $public_id ID. @param string $status Status. @return array<string,mixed>|WP_Error */
