...
final class SUN_Admin {
	...
	/** @return void */ public function bulk_preview(){check_admin_referer('sun_bulk_preview');$ids=preg_split('/[\s,]+/',sanitize_textarea_field(wp_unslash($_POST['user_ids']??'')));$result=$this->bulk->preview((array)$ids,array('title'=>sanitize_text_field(wp_unslash($_POST['title']??'')),'summary'=>sanitize_textarea_field(wp_unslash($_POST['summary']??'')),'priority'=>sanitize_key($_POST['priority']??'normal'),'deep_link'=>esc_url_raw(wp_unslash($_POST['deep_link']??'')),'reason'=>sanitize_textarea_field(wp_unslash($_POST['reason']??'')),'compensation_plan'=>sanitize_textarea_field(wp_unslash($_POST['compensation_plan']??''))));if(is_wp_error($result)){wp_die(esc_html($result->get_error_message()));}set_transient('sun_bulk_preview_'.get_current_user_id(),$result,10*MINUTE_IN_SECONDS);wp_safe_redirect(admin_url('admin.php?page=sabri-notifications&notice=bulk-preview'));exit;}
