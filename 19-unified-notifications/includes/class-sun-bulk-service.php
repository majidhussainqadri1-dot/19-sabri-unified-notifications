...
final class SUN_Bulk_Service {
	...
	public function preview( array $user_ids, array $event ) {
		global $wpdb;
		if ( ! $this->auth->can_send_bulk() ) { return new WP_Error('sun_bulk_forbidden',__('Founder approval is required for bulk notices.','sabri-unified-notifications'),array('status'=>403)); }
		...
		$data=array(
			'user_ids'=>$user_ids,
			'event'=>array(
				'owner'=>sanitize_text_field((string)($event['owner']??'File19')),
	...
	public function confirm( $public_id, $confirmation ) {
		global $wpdb;
		if(!$this->auth->can_send_bulk()){return new WP_Error('sun_bulk_forbidden',__('Founder approval is required.','sabri-unified-notifications'),array('status'=>403));}
		...
		$wpdb->update(SUN_Database::table('bulk_jobs'),array('status'=>'queued','updated_at'=>SUN_Database::now()),array('id'=>(int)$row['id'])); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery
	...
	public function cancel($public_id){global $wpdb;if(!$this->auth->can_send_bulk()){return new WP_Error('sun_bulk_forbidden',__('Founder approval is required.','sabri-unified-notifications'),array('status'=>403));}$updated=$wpdb->query($wpdb->prepare("UPDATE ".SUN_Database::table('bulk_jobs')." SET cancel_requested=1,status=IF(status='preview','cancelled',status),updated_at=%s WHERE public_id=%s AND status IN ('preview','queued','processing')",SUN_Database::now(),sanitize_text_field($public_id))); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery
