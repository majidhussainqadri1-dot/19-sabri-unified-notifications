...
final class SUN_Health {
	...
	public function snapshot() {
		...
		$metrics = array(
			'queued'               => ! empty( $tables['deliveries'] ) ? (int) $wpdb->get_var( "SELECT COUNT(*) FROM {$queue} WHERE status='queued'" ) : 0,
			'failed'               => ! empty( $tables['deliveries'] ) ? (int) $wpdb->get_var( "SELECT COUNT(*) FROM {$queue} WHERE status='failed'" ) : 0,
			'expired'              => ! empty( $tables['deliveries'] ) ? (int) $wpdb->get_var( "SELECT COUNT(*) FROM {$queue} WHERE status='expired'" ) : 0,
			'dead_letter'          => ! empty( $tables['dead_letters'] ) ? (int) $wpdb->get_var( "SELECT COUNT(*) FROM {$dead} WHERE status='open'" ) : 0,
			'oldest_queue_seconds' => 0,
			'attention_profiles'   => ! empty( $tables['attention_profiles'] ) ? (int) $wpdb->get_var( 'SELECT COUNT(*) FROM ' . SUN_Database::table( 'attention_profiles' ) ) : 0,
			'automation_rules'     => ! empty( $tables['notification_rules'] ) ? (int) $wpdb->get_var( 'SELECT COUNT(*) FROM ' . SUN_Database::table( 'notification_rules' ) . ' WHERE enabled=1' ) : 0,
			'active_experiments'   => ! empty( $tables['experiments'] ) ? (int) $wpdb->get_var( "SELECT COUNT(*) FROM " . SUN_Database::table( 'experiments' ) . " WHERE status='running'" ) : 0,
		);
		if ( ! empty( $tables['deliveries'] ) ) {
		...
		$checks = array(
			...
			'file00_contract'              => class_exists( 'SMC_Contracts' ) && is_callable( array( 'SMC_Contracts', 'assertions' ) ),
			'file02_step_up_provider'      => class_exists( 'SAUTH_Passkey_Runtime' ) && is_callable( array( 'SAUTH_Passkey_Runtime', 'current_assurance' ) ),
			'file01_registry'              => ! empty( $cross['file01_registry'] ),
			'file20_single_bell'           => ! empty( $cross['file20_single_bell'] ),
			'file24_containment_contract'  => false !== has_filter( 'sun_file24_notification_containment_active' ),
			...
			'attention_os'                 => class_exists( 'SUN_Attention_Service' ),
			'automation'                   => class_exists( 'SUN_Automation_Service' ),
			'routing'                      => class_exists( 'SUN_Routing_Service' ),
			'experiments'                  => class_exists( 'SUN_Experiments_Service' ),
			'trace'                        => class_exists( 'SUN_Trace_Service' ),
