...
final class SUN_Policy_Engine {
	...
	public function __construct( SUN_Preferences $preferences, SUN_Producer_Registry $registry, SUN_Subscriptions $subscriptions ) {
		$this->preferences=$preferences;$this->registry=$registry;$this->subscriptions=$subscriptions;
	}

	/**
	 * Producer envelope values are hints only. They may raise priority/sensitivity
	 * but cannot lower policy protection or impersonate another category.
	 *
	...
	public function decide( array $event, array $recipient ) {
		...
		if(!$policy){return new WP_Error('sun_policy_missing',__('No active notification policy matches this event.','sabri-unified-notifications'));}
		...
		if(empty($subscription['allowed'])){
			return array('suppressed'=>true,'suppress_reason'=>'subscription_preference','category'=>$category,'priority'=>$priority,'mandatory'=>false,'sensitivity'=>$sensitivity,'channels'=>array(),'deliveries'=>array());
		}
		...
		if(!empty($recipient['channels'])){$channels=array_values(array_intersect($channels,$recipient['channels']));}
		if(!in_array('in_app',$channels,true)){array_unshift($channels,'in_app');}
		$deliveries=array();
		foreach(array_unique($channels) as $channel){
			$pref=$this->preferences->get($recipient['user_id'],$category,$channel);
			if('in_app'!==$channel&&empty($pref['enabled'])&&!($mandatory&&in_array($channel,array('email','push'),true))){continue;}
			if('in_app'===$channel){continue;}
			$base=$this->preferences->next_delivery_time($recipient['user_id'],$category,$channel,$mandatory);
			if($mandatory||!$policy['digest_allowed']){$digest=array('time'=>$base,'key'=>null);}
			elseif(!empty($subscription['frequency'])){$digest=$this->digest_for_frequency((string)$subscription['frequency'],$base,(string)$pref['timezone']);}
			else{$digest=$this->preferences->digest_schedule($recipient['user_id'],$category,$channel,$base);}
			$deliveries[]=array('channel'=>$channel,'scheduled_at'=>$digest['time']->format('Y-m-d H:i:s'),'digest_key'=>$digest['key']);
		}
		return array(
			'suppressed'=>false,'policy_key'=>$policy['policy_key'],'policy_version'=>$policy['version'],'category'=>$category,'priority'=>$priority,'mandatory'=>$mandatory,'sensitivity'=>$sensitivity,
			'digest_allowed'=>(bool)$policy['digest_allowed'],'subscription_frequency'=>$subscription['frequency'],'channels'=>$channels,'deliveries'=>$deliveries,
		);
	}
	...
	private function stronger_value( $base, $hint, array $ordered, $fallback ) {
		$base=sanitize_key($base);$hint=sanitize_key($hint);$base_index=array_search($base,$ordered,true);$hint_index=array_search($hint,$ordered,true);
		if(false===$base_index){$base_index=array_search($fallback,$ordered,true);}if(false===$hint_index){return $ordered[$base_index];}return $ordered[max($base_index,$hint_index)];
	}
	...
	private function digest_for_frequency( $frequency, DateTimeImmutable $base, $timezone ) {
		if('immediate'===$frequency){return array('time'=>$base,'key'=>null);}
		try{$tz=new DateTimeZone($timezone?:'UTC');}catch(Exception $e){$tz=new DateTimeZone('UTC');}
		$local=$base->setTimezone($tz);
		if('daily'===$frequency){$next=$local->setTime(8,0);if($next<=$local){$next=$next->modify('+1 day');}return array('time'=>$next->setTimezone(new DateTimeZone('UTC')),'key'=>'daily:'.$next->format('Y-m-d'));}
		$next=$local->modify('next monday')->setTime(8,0);return array('time'=>$next->setTimezone(new DateTimeZone('UTC')),'key'=>'weekly:'.$next->format('o-W'));
	}
}
