...
final class SUN_Attention_Service {
    ...
    public function live_update( $public_id, array $patch, $producer = '' ) {
        global $wpdb; $producer = sanitize_key( (string) $producer ); if ( '' === $producer ) { return new WP_Error( 'sun_live_owner_required', __( 'The canonical producer identity is required for a live notification update.', 'sabri-unified-notifications' ), array( 'status' => 403 ) ); } $note = $wpdb->get_row( $wpdb->prepare( 'SELECT id,recipient_id,producer,version,status,expires_at FROM ' . SUN_Database::table( 'notifications' ) . ' WHERE public_id=%s LIMIT 1', sanitize_text_field( $public_id ) ), ARRAY_A ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery
        if ( ! $note || in_array( $note['status'], array( 'deleted', 'expired' ), true ) || ( ! empty( $note['expires_at'] ) && $note['expires_at'] <= SUN_Database::now() ) ) { return new WP_Error( 'sun_live_notification_not_found', __( 'The live notification is unavailable.', 'sabri-unified-notifications' ) ); }
        if ( ! hash_equals( sanitize_key( (string) $note['producer'] ), $producer ) ) { return new WP_Error( 'sun_live_owner_mismatch', __( 'Only the canonical producer may update this notification projection.', 'sabri-unified-notifications' ), array( 'status' => 403 ) ); }
    ...
    public function update_device_profile( $user_id, $device_public_id, array $input ) {
        ...
        $categories = array_values( array_intersect( array( 'security','safety','clinic','publishing','learning','research','knowledge','search','analytics','social','marketplace','messages','media','system','marketing' ), array_map( 'sanitize_key', (array) ( $input['categories'] ?? array() ) ) ) );
