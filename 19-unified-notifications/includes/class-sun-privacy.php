...
final class SUN_Privacy {
	...
	public function export($email,$page=1){
		global $wpdb;$user=get_user_by('email',$email);if(!$user){return array('data'=>array(),'done'=>true);}$limit=100;$offset=max(0,absint($page)-1)*$limit;
		$rows=$wpdb->get_results($wpdb->prepare('SELECT public_id,category,priority,title,summary,status,created_at,read_at,archived_at FROM '.SUN_Database::table('notifications').' WHERE recipient_id=%d ORDER BY id ASC LIMIT %d OFFSET %d',$user->ID,$limit,$offset),ARRAY_A);$data=array();foreach($rows as $row){$data[]=$this->export_item('sabri-notifications',__('Notifications','sabri-unified-notifications'),'notification-'.$row['public_id'],$row);} // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery
		if(1===max(1,absint($page))){
			$prefs=$wpdb->get_results($wpdb->prepare('SELECT category,channel,enabled,digest_frequency,quiet_enabled,quiet_start,quiet_end,timezone,consent_source,consent_at,created_at,updated_at FROM '.SUN_Database::table('preferences').' WHERE user_id=%d ORDER BY id ASC',$user->ID),ARRAY_A);foreach($prefs as $index=>$pref){$data[]=$this->export_item('sabri-notification-preferences',__('Notification Preferences','sabri-unified-notifications'),'preference-'.($index+1),$pref);} // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery
			$subs=$wpdb->get_results($wpdb->prepare('SELECT public_id,scope_type,scope_id,enabled,frequency,created_at,updated_at FROM '.SUN_Database::table('subscriptions').' WHERE user_id=%d ORDER BY scope_type,scope_id',$user->ID),ARRAY_A);foreach($subs as $sub){$data[]=$this->export_item('sabri-notification-subscriptions',__('Notification Subscriptions','sabri-unified-notifications'),'subscription-'.$sub['public_id'],$sub);} // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery
			$devices=$wpdb->get_results($wpdb->prepare('SELECT public_id,provider,platform,status,last_seen_at,expires_at,created_at,updated_at FROM '.SUN_Database::table('devices').' WHERE user_id=%d ORDER BY id ASC',$user->ID),ARRAY_A);foreach($devices as $device){$data[]=$this->export_item('sabri-notification-devices',__('Notification Devices','sabri-unified-notifications'),'device-'.$device['public_id'],$device);} // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery
			$deliveries=$wpdb->get_results($wpdb->prepare('SELECT public_id,channel,provider,status,attempt_count,scheduled_at,accepted_at,delivered_at,created_at,updated_at FROM '.SUN_Database::table('deliveries').' WHERE recipient_id=%d ORDER BY id ASC LIMIT 500',$user->ID),ARRAY_A);foreach($deliveries as $delivery){$data[]=$this->export_item('sabri-notification-deliveries',__('Notification Delivery History','sabri-unified-notifications'),'delivery-'.$delivery['public_id'],$delivery);} // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery
		}
	...
	public function erase($email,$page=1){
		...
		$wpdb->delete($devices,array('user_id'=>$user->ID),array('%d'));$wpdb->delete($prefs,array('user_id'=>$user->ID),array('%d'));$wpdb->delete($subs,array('user_id'=>$user->ID),array('%d')); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery
	...
	/** @param string $group_id Group ID. @param string $group_label Label. @param string $item_id Item ID. @param array<string,mixed> $row Row. @return array<string,mixed> */
	private function export_item($group_id,$group_label,$item_id,array $row){$data=array();foreach($row as $key=>$value){$data[]=array('name'=>ucwords(str_replace('_',' ',$key)),'value'=>(string)$value);}return array('group_id'=>$group_id,'group_label'=>$group_label,'item_id'=>$item_id,'data'=>$data);}
}
