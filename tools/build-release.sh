...
source=Path(sys.argv[1]).resolve();archive=Path(sys.argv[2]).resolve();base=source.name;fixed=(2026,9,24,0,0,0);paths=sorted(source.rglob('*'),key=lambda p:p.relative_to(source).as_posix())
