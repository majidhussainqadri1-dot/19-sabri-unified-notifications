#!/usr/bin/env bash
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)";PLUGIN="$ROOT/19-unified-notifications";BUILD="$ROOT/build";STAGE="$BUILD/stage";NAME="19-sabri-unified-notifications-2.4.0.zip";CANONICAL="unified-notifications-19"
...
source=Path(sys.argv[1]).resolve();archive=Path(sys.argv[2]).resolve();base=source.name;fixed=(2026,8,7,0,0,0);paths=sorted(source.rglob('*'),key=lambda p:p.relative_to(source).as_posix())
