...
mkdir -p "$STAGE"
cp -a "$PLUGIN" "$STAGE/$FOLDER"

(
  cd "$STAGE"
  find "$FOLDER" -type f ! -name MANIFEST.sha256 -print0 \
    | LC_ALL=C sort -z \
    | xargs -0 sha256sum > "$FOLDER/MANIFEST.sha256"
)

python3 - "$STAGE/$FOLDER" "$BUILD/$NAME" <<'PY'
...
[[ "$(unzip -Z1 "$BUILD/$NAME" | head -n1)" == "$FOLDER/" ]] || {
